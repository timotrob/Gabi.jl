...
let libsvm = C_NULL
    ...
    function get_libsvm()
        if libsvm == C_NULL
          @windows_only begin
              libsvm = Libdl.dlopen(joinpath(Pkg.dir(), "Gabi", "deps","windows","libsvm-3.21","libsvm.dll"))
          end
          @unix_only begin
              libsvm = Libdl.dlopen(joinpath(Pkg.dir(), "Gabi", "deps","unix","libsvm-3.21","libsvm.so.2"))
          end
...
function svmtrain{U<:Real,T}(labels::Vector{T},
        instances::Matrix{U}; svm_type::Int32=CSVC,
        kernel_type::Int32=RBF, degree::Integer=3,
        gamma::Float64=1.0/size(instances, 2), coef0::Float64=0.0,
...
function svmpredict{U<:Real,T}(model::SVMModel{T},
        instances::Matrix{U})
    global verbosity
    ninstances = size(instances, 2)
